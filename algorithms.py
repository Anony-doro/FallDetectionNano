...
def extract_keypoints_parallel(queue, args, self_counter, other_counter, consecutive_frames, event):
    ...
    while not event.is_set():
        ...
        hsv_img = cuda_pipeline.process_frame(img, width, height)
        ...
        if args.coco_points:
            ...
            # Create masks for histogram calculation
            uhist_list = []
            lhist_list = []
            for ubbox, lbbox in zip(ubboxes, lbboxes):
                # Create mask for upper body
                mask = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
                cv2.fillPoly(mask, [ubbox], 1)
                uhist = cuda_pipeline.calculate_histogram(hsv_img, mask)
                uhist_list.append(uhist)
                
                # Create mask for lower body
                mask = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
                cv2.fillPoly(mask, [lbbox], 1)
                lhist = cuda_pipeline.calculate_histogram(img, mask)
...
def remove_wrongly_matched(matched_1, matched_2):
    ...
    for ip1, ip2 in zip(matched_1, matched_2):
        ...
        correlation = cuda_pipeline.compare_histograms(
